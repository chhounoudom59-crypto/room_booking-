...
class CustomUserAdmin(UserAdmin):
    ...
    list_display = ("username", "email", "get_full_name", "user_type", "is_approved", "is_active", "date_joined")

    list_filter = ("user_type", "is_approved", "is_active", "is_superuser", "date_joined")
    ...
    add_fieldsets = (
        *UserAdmin.add_fieldsets,
        ("Additional Info", {"fields": ("user_type", "student_id", "phone_number", "department", "is_approved")}),
    )
...
class RoomAdmin(admin.ModelAdmin):
    ...
    def room_image_thumb(self, obj):
        if obj.image:
            return format_html(
                '<img src="{}" style="height:48px; width:auto; border-radius:6px; object-fit:cover;" />', obj.image.url
            )
    ...
    list_filter = ("room_type", "availability_status", "is_available", "created_at")
    ...
    fieldsets = (
        ("Basic Information", {"fields": ("name", "room_number", "room_type", "capacity", "image")}),
        ("Details", {"fields": ("description", "equipment")}),
        ("Status", {"fields": ("availability_status", "is_available", "auto_status_updates")}),
        ("Timestamps", {"fields": ("created_at", "updated_at"), "classes": ("collapse",)}),
    )
    ...
    def is_bookable_status(self, obj):
        ...
        if obj.is_bookable():
            return format_html('<span style="color: green; font-weight: bold;">✓ Bookable</span>')
        else:
            return format_html('<span style="color: red; font-weight: bold;">✗ Not Bookable</span>')
...
class BookingAdmin(admin.ModelAdmin):
    ...
    list_display = ("room", "user", "start_time", "end_time", "status", "duration_display", "is_active_status")

    list_filter = ("status", "room__room_type", "start_time", "created_at")
    ...
    fieldsets = (
        ("Booking Details", {"fields": ("user", "room", "start_time", "end_time")}),
        ("Purpose & Notes", {"fields": ("purpose", "additional_notes")}),
        ("Status", {"fields": ("status",)}),
        ("Information", {"fields": ("duration_hours", "created_at", "updated_at"), "classes": ("collapse",)}),
    )
...
class BookingRuleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Rule Name", {"fields": ("name", "is_active")}),
        (
            "Booking Limits",
            {
                "fields": (
        ...
        (
            "Time Constraints",
            {"fields": ("max_advancee_days", "min_advance_hours", "booking_start_time", "booking_end_time")},
        ),
        ("Timestamps", {"fields": ("created_at", "updated_at"), "classes": ("collapse",)}),
    )
...
class AnnouncementAdmin(admin.ModelAdmin):
    ...
    list_filter = ("announcement_type", "priority", "is_active", "created_at")
    ...
    fieldsets = (
        ...
        ("Classification", {"fields": ("announcement_type", "priority")}),
        ("Visibility", {"fields": ("is_active", "show_until")}),
