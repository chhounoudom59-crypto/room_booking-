...
def api_list_rooms(request):
    try:
        ...
        return JsonResponse({"success": True, "count": len(rooms_data), "rooms": rooms_data})
...
def api_check_availability(request):
    try:
        ...
        if not all([room_id, date_str, start_time_str, end_time_str]):
            return JsonResponse(
                {"success": False, "error": "Missing required parameters: room_id, date, start_time, end_time"},
                status=400,
            )
        ...
        if start_datetime >= end_datetime:
            return JsonResponse(
                {"success": False, "available": False, "error": "End time must be after start time"}, status=400
            )
        ...
        if not is_available:
            conflicting_bookings = Booking.objects.filter(
                room=room, start_time__lt=end_datetime, end_time__gt=start_datetime, status="confirmed"
            )
        ...
        return JsonResponse(
            {
                ...
                "room": {"id": room.id, "name": room.name, "room_number": room.room_number, "capacity": room.capacity},
                "requested_time": {
...
def api_create_booking(request):
    try:
        ...
        try:
            ...
            return JsonResponse({"success": False, "error": f"User with email {user_email} not found"}, status=404)

        if not user.is_active:
            return JsonResponse(
                {"success": False, "error": "Your account is deactivated. Contact an administrator."}, status=403
            )
        ...
        if not agreed_to_policy:
            return JsonResponse(
                {"success": False, "error": "You must agree to the room usage policy before booking."}, status=400
            )
        ...
        try:
            ...
            return JsonResponse({"success": False, "error": f"Room with ID {room_id} not found"}, status=404)
        ...
        if start_datetime < now:
            return JsonResponse({"success": False, "error": "Cannot book rooms in the past"}, status=400)
        ...
        if duration_hours < BOOKING_MIN_DURATION_HOURS:
            return JsonResponse(
                {"success": False, "error": f"Minimum booking duration is {BOOKING_MIN_DURATION_HOURS} hour"},
                status=400,
            )

        if duration_hours > BOOKING_MAX_DURATION_HOURS:
            return JsonResponse(
                {"success": False, "error": f"Maximum booking duration is {BOOKING_MAX_DURATION_HOURS} hours"},
                status=400,
            )
        ...
        if advance_days > BOOKING_MAX_ADVANCE_DAYS:
            return JsonResponse(
                {"success": False, "error": f"Cannot book more than {BOOKING_MAX_ADVANCE_DAYS} days in advance"},
                status=400,
            )
        ...
        if advance_hours < BOOKING_MIN_ADVANCE_HOURS:
            return JsonResponse(
                {"success": False, "error": f"Must book at least {BOOKING_MIN_ADVANCE_HOURS} hour in advance"},
                status=400,
            )
        ...
        if not room.is_available or getattr(room, "availability_status", "available") != "available":
            return JsonResponse(
                {"success": False, "error": "Room is currently unavailable based on admin settings"}, status=400
            )
        ...
        if not can_book_consecutively:
            return JsonResponse({"success": False, "error": consecutive_error}, status=400)
        ...
        return JsonResponse({"success": False, "error": " ".join(messages) or "Booking validation failed."}, status=400)
    except Exception as e:
        return JsonResponse({"success": False, "error": str(e)}, status=400)
...
def api_list_user_bookings(request):
    try:
        ...
        if not user_email:
            return JsonResponse({"success": False, "error": "user_email parameter is required"}, status=400)
...
def api_cancel_booking(request):
    try:
        ...
        if not all([booking_id, user_email]):
            return JsonResponse(
                {"success": False, "error": "Missing required fields: booking_id, user_email"}, status=400
            )
        ...
        try:
            ...
            return JsonResponse({"success": False, "error": f"Booking with ID {booking_id} not found"}, status=404)

        # Verify user owns the booking
        if booking.user.email != user_email:
            return JsonResponse({"success": False, "error": "You can only cancel your own bookings"}, status=403)
        ...
        if time_until_booking.total_seconds() <= 0:
            return JsonResponse(
                {"success": False, "error": "This booking has already started or passed and cannot be cancelled"},
                status=400,
            )
...
def api_search_rooms(request):
    try:
        ...
        rooms_data = [_serialize_room(room, request) for room in rooms[:10]]

        return JsonResponse({"success": True, "query": query, "count": len(rooms_data), "rooms": rooms_data})
