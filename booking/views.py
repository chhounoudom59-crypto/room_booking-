...
def room_detail(request, room_id):
    ...
    upcoming_bookings = Booking.objects.filter(
        room=room, start_time__date__range=[today, next_week], status__in=["confirmed"]
    ).order_by("start_time")
...
def room_create(request):
    ...
    context = {"form": form, "title": "Create New Room", "button_text": "Create Room"}
...
def room_edit(request, room_id):
    ...
    context = {"form": form, "room": room, "title": f"Edit Room: {room.name}", "button_text": "Update Room"}
...
def room_toggle_status(request, room_id):
    ...
    if request.method == "POST":
        ...
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"success": True, "status": status, "is_available": room.is_available})
...
def check_room_availability(request):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            return JsonResponse(
                {"available": is_available, "room_available": room.is_available, "conflicts": conflict_details}
            )
...
def user_bookings(request):
    ...
    if search_query:
        bookings = bookings.filter(Q(room__name__icontains=search_query) | Q(purpose__icontains=search_query))
...
def modify_booking(request, booking_id):
    ...
    return render(request, "UserPage/booking.html", {"form": form, "booking": booking})


@login_required
def booking_calendar(request):
    bookings = Booking.objects.filter(user=request.user, status__in=["pending", "confirmed"]).select_related("room")
    ...
    return render(request, "UserPage/booking.html", {"calendar_events": calendar_events})
...
def check_availability(request):
    try:
        ...
        if not room.is_available:
            return JsonResponse({"available": False, "message": "Room is currently unavailable"})

        # Check for booking conflicts
        conflicts = Booking.objects.filter(room=room, date=booking_date, status__in=["confirmed", "pending"]).filter(
        ...
        if not rules_check["valid"]:
            return JsonResponse({"available": False, "message": rules_check["message"]})

        return JsonResponse({"available": True, "message": "Room is available for booking"})

    except Exception as e:
        return JsonResponse({"available": False, "message": f"Error checking availability: {e!s}"})
...
def check_conflicts(request):
    try:
        ...
        nearby_bookings = (
            Booking.objects.filter(room=room, date=booking_date, status__in=["confirmed", "pending"])
            .filter(Q(start_time__gte=extended_start) | Q(end_time__lte=extended_end))
...
def get_suggested_times(room, date, preferred_start, preferred_end):
    ...
    existing_bookings = Booking.objects.filter(room=room, date=date, status__in=["confirmed", "pending"]).order_by(
        "start_time"
    )
    ...
    for booking in existing_bookings:
        ...
        if current_time < booking.start_time:
            potential_end = min(
                (datetime.combine(date, current_time) + timedelta(minutes=duration_minutes)).time(), booking.start_time
            )

            if (
                datetime.combine(date, potential_end) - datetime.combine(date, current_time)
            ).total_seconds() >= duration_minutes * 60:
                suggested_times.append(
                    {"start_time": current_time.strftime("%H:%M"), "end_time": potential_end.strftime("%H:%M")}
                )
    ...
    if current_time < operating_end and len(suggested_times) < 3:
        potential_end = min(
            (datetime.combine(date, current_time) + timedelta(minutes=duration_minutes)).time(), operating_end
        )
...
def check_booking_rules(user, room, start_datetime, end_datetime):
    try:
        ...
        if duration.total_seconds() / 3600 > rules.max_duration_hours:
            return {"valid": False, "message": f"Booking duration cannot exceed {rules.max_duration_hours} hours"}

        # Check daily limit
        today = start_datetime.date()
        today_bookings = Booking.objects.filter(user=user, date=today, status__in=["confirmed"]).count()

        if today_bookings >= rules.daily_limit:
            return {"valid": False, "message": f"Daily booking limit of {rules.daily_limit} bookings exceeded"}
        ...
        weekly_bookings = Booking.objects.filter(
            user=user, date__range=[week_start, week_end], status__in=["confirmed"]
        ).count()

        if weekly_bookings >= rules.weekly_limit:
            return {"valid": False, "message": f"Weekly booking limit of {rules.weekly_limit} bookings exceeded"}
        ...
        if advance_days > rules.advance_booking_days:
            return {"valid": False, "message": f"Cannot book more than {rules.advance_booking_days} days in advance"}

        return {"valid": True}

    except Exception as e:
        return {"valid": False, "message": f"Error checking booking rules: {e!s}"}
...
def user_dashboard(request):
    ...
    # Get user's bookings
    Booking.objects.filter(user=user, start_time__gt=timezone.now(), status__in=["confirmed"]).order_by("start_time")[
        :5
    ]

    past_bookings = Booking.objects.filter(user=user, end_time__lt=timezone.now()).order_by("-end_time")[:5]
    ...
    # Get available rooms
    available_rooms = Room.objects.filter(is_available=True, availability_status="available").count()

    # Recent activity
    Booking.objects.filter(user=user).order_by("-created_at")[:10]
...
def room_detail(request, room_id):
    ...
    today = timezone.now().date()
    today_bookings = Booking.objects.filter(room=room, start_time__date=today, status__in=["confirmed"]).order_by(
        "start_time"
    )
...
def create_booking(request):
            ...
            try:
                ...
                try:
                    ...
                    social_token = SocialToken.objects.filter(
                        account=social_account, account__user=request.user, account__provider="google"
                    ).first()
                    if social_account and social_token:
                        creds = Credentials(
                            ...
                            else settings.SOCIALACCOUNT_PROVIDERS["google"]["APP"]["client_id"],
                            client_secret=settings.SOCIAL_AUTH_GOOGLE_OAUTH2_SECRET
                            if hasattr(settings, "SOCIAL_AUTH_GOOGLE_OAUTH2_SECRET")
                            else settings.SOCIALACCOUNT_PROVIDERS["google"]["APP"]["secret"],
                        )
    ...
    buildings_list = [
        ...
        for type_key in used_types
        if type_key in dict(building_types)
    ]
...
def booking_calendar(request):
    ...
    bookings = Booking.objects.filter(
        status="confirmed", start_time__gte=timezone.now() - timedelta(days=30)
...
def rooms_api_availability(request):
    ...
    if date_str:
        try:
            ...
            return JsonResponse({"success": True, "date": date_str, "rooms": room_data})
...
def booking_schedule_by_day(request):
    ...
    if date_str:
        ...
        date = timezone.now().date()
    bookings = (
        Booking.objects.filter(user=user, start_time__date=date)
        .select_related("room")
...
def check_room_availability_ajax(request):
    ...
    try:
        ...
        if not all([room_id, date_str, start_time_str, end_time_str]):
            return JsonResponse({"available": False, "error": "Missing required parameters"})
        ...
        if start_datetime <= timezone.now():
            return JsonResponse({"available": False, "error": "Cannot book time in the past", "conflicts": []})

        if start_time >= end_time:
            return JsonResponse({"available": False, "error": "Start time must be before end time", "conflicts": []})
        ...
        return JsonResponse({"available": False, "error": f"Invalid date/time format: {e!s}"})
    except Exception as e:
        return JsonResponse({"available": False, "error": f"Server error: {e!s}"})
