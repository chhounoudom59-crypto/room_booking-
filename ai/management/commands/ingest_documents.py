...
from ai.ingest_documents import clear_collection, ingest_document
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--file", type=str, required=True, help="Path to document file (relative to project root)")
        parser.add_argument(
            "--collection",
            type=str,
            required=True,
        ...
        parser.add_argument("--verify", action="store_true", help="Verify ingestion by checking collection stats")
    ...
    def handle(self, *args, **options):
        ...
        success = ingest_document(
            file_path=file_path, collection_name=collection_name, chunk_size=chunk_size, chunk_overlap=chunk_overlap
        )
