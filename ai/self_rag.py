...
class SelfRAG:
    ...
    def generate_with_reflection(
        ...
        relevant_doc_ids_in_corpus: List[str] | None = None,
    ) -> Dict:
        ...
        while iteration < max_iterations:
            ...
            # GENERATION
            # =========================
            response = self._generate_response(query, retrieved_docs, entities, intent, context)
            ...
            success = (
                ...
                and context_precision >= self.thresholds["context_precision"]
                ...
                and routing_accuracy >= self.thresholds["routing_accuracy"]
            )
    ...
    def _compute_context_recall(
        ...
        retrieved_set = {self._extract_doc_id(d) for d in (retrieved_docs or []) if self._extract_doc_id(d)}
    ...
    def _compute_context_precision_proxy(self, retrieved_docs: List[Dict]) -> float:
        ...
        relevant = sum(1 for d in retrieved_docs if float(d.get("score", 0.0) or 0.0) > 0.5)
    ...
    def _compute_faithfulness(self, response: str, docs: List[Dict]) -> float:
        ...
        doc_context = "\n".join(d.get("document") or d.get("text", "") for d in docs)
...
- 0.0 = completely irrelevant"""
    ...
    def _compute_completeness(self, response: str, entities: Dict | None = None) -> float:
...
- 0.0 = completely incomplete"""
    ...
    def get_pipeline_metrics(self) -> Dict:
        ...
        return {
            ...
            "iteration_efficiency": 1.0 / (self.pipeline_metrics["total_iterations"] / total),
        }
    ...
    def _refine_query(self, query: str, entities: Dict | None = None, intent=None) -> str:
        refined = query
    ...
    def _generate_response(
        ...
        doc_context = "\n".join((d.get("document") or d.get("text", "")) for d in retrieved_docs[:3])
