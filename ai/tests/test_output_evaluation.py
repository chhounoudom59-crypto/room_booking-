...
class MockRetriever:
    ...
    def retrieve(self, query: str, **kwargs) -> List[Dict]:
        ...
        return [
            ...
            {"document": "Conference rooms can accommodate up to 100 people.", "text": "room capacity", "score": 0.85},
            {"document": "Room booking must be done 24 hours in advance.", "text": "advance booking", "score": 0.7},
        ]
...
def test_faithfulness_evaluation():
    ...
    for test in test_cases:
        ...
        if (
            (test["expected"] == "high" and score > 0.75)
            or (test["expected"] == "medium" and 0.4 < score < 0.75)
            or (test["expected"] == "low" and score < 0.4)
        ):
            print("    ✓ Pass")
        else:
            print("    ✗ Mismatch")
    ...
    test_cases = [
        {
            ...
            "entities": {"room": "A2", "date": "Monday", "time": "2 PM", "capacity": "50", "duration": "8 hours"},
            "expected": "high",
...
def test_reflection_pipeline():
    ...
    result = self_rag.generate_with_reflection(query=query, entities=entities, intent="booking", max_iterations=3)
...
def test_threshold_sensitivity():
    ...
    for config in threshold_configs:
        ...
        result = self_rag.generate_with_reflection(
            query="Book room A2 for 50 people", entities={"room": "A2", "capacity": "50"}, max_iterations=3
        )
