...
class MockInputModel:
    ...
    def __call__(self, prompt: str) -> str:
        ...
        if "Classify the intent" in prompt or "intent" in prompt.lower():
            return json.dumps(
                {
                    "intent": {"primary": "booking", "confidence": 0.95, "secondary": None},
                    "entities": {"room": "A2", "capacity": "50", "date": "Monday", "time": "2 PM"},
                    "routing": {
                        "complexity_score": 2.5,
                        "routing_strategy": "HYBRID_RETRIEVAL",
                        "execution_plan": {"decompose_query": False, "expand_query": True},
                    },
                    "sub_queries": None,
...
def test_intent_classification():
    ...
    for query in test_queries:
        ...
        print("  ✓ Pass" if intent.get("confidence", 0) > 0.8 else "  ✗ Low confidence")
...
def test_entity_extraction():
    ...
    test_cases = [
        ...
        {"query": "Is conference room B5 available?", "expected_entities": ["room"]},
    ]
    ...
    for test in test_cases:
        ...
        print("  ✓ Pass" if coverage >= 50 else "  ✗ Low coverage")
...
def test_complexity_scoring():
    ...
    for query, expected_complexity, description in test_cases:
        ...
        print("  ✓ Pass" if abs(complexity - expected_complexity) <= 1 else "  ✗ Mismatch")
...
def test_routing_strategy():
    ...
    input_model = MockInputModel()
    QueryProcessor(llm_client=input_model)
    ...
    for description, complexity in test_cases:
        ...
        if complexity >= 3:
            print("  Strategy: MULTI_QUERY_RETRIEVAL (Generate 3 query variations)")
            print("  Top-K: 10 (Kx2 candidates)")
        else:
            print("  Strategy: HYBRID_RETRIEVAL (Vector + Keyword)")
            print("  Top-K: 5")
