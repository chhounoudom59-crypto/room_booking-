...
def test_pipeline_complexity_routing():
    ...
    rag = AgenticRAG(llm_client=input_model, eval_llm_client=output_model, enable_multi_query=True)
...
def test_pipeline_quality_iteration():
    ...
    rag = AgenticRAG(llm_client=input_model, eval_llm_client=output_model, enable_self_rag=True)
    ...
    for i, query in enumerate(queries, 1):
        ...
        try:
            ...
            if "evaluation_scores" in result:
                ...
                if overall >= 0.7:
                    print("    ✓ High quality - Accepted on first attempt")
                elif overall >= 0.5:
                    print("    ⚠ Medium quality - May require refinement")
                else:
                    print("    ✗ Low quality - Needs iteration")
            else:
...
def test_pipeline_error_handling():
    ...
    try:
        ...
        print("    ✓ Pipeline continues without LLM")
    ...
    try:
        ...
        print("    ✓ Pipeline works with all features disabled")
    except Exception as e:
        print(f"    ✗ Error: {e}")
