...
from chatbot.services.booking_service import auto_book, build_booking_criteria, find_best_rooms
...
async def handle_user_profile(user):
    ...
    try:
        ...
        return {"success": True, "response_text": profile_text, "profile_data": user_data}

    except Exception as e:
        logger.exception(f"Profile query failed: {e}")
        return {"success": False, "message": f"Could not retrieve profile: {e!s}"}
...
async def handle_user_history(user):
    ...
    try:
        ...
        # Get user's bookings
        all_bookings = await sync_to_async(lambda: list(Booking.objects.filter(user=user).order_by("-start_time")))()
        ...
        return {"success": False, "message": f"Could not retrieve booking history: {e!s}"}
    ...
    try:
        if agent and hasattr(agent, "room_plugin") and agent.room_plugin:
            ...
            if hasattr(agent, "booking_automation") and agent.booking_automation:
                return await sync_to_async(agent.booking_automation.find_best_rooms)(criteria, limit=limit)
    ...
    if not validation.get("valid"):
        ...
        if missing and llm_client:
            friendly_msg = await sync_to_async(_ask_llm_clarification)(
                missing_fields=missing, criteria=criteria, user_message=user_message, llm_client=llm_client
            )
        else:
...
async def handle_confirm_booking(user, criteria: dict):
    ...
    # Fallback
    return await auto_book(agent.booking_automation if agent else None, user, criteria)
...
async def handle_availability_query(entities: dict, user_message: str):
    ...
    has_time_filter = bool(criteria.get("date") and criteria.get("start_time") and criteria.get("end_time"))
...
async def handle_modify_booking(user, entities: dict, session_ctx: dict):
    ...
    is_auth = await sync_to_async(lambda: user.is_authenticated if user else False)()
    if not is_auth:
        return {"success": False, "message": "Please log in to modify your booking."}
    ...
    try:
        ...
        latest_booking = await sync_to_async(
            lambda: (
                Booking.objects.filter(user=user, status="confirmed", start_time__gt=timezone.now())
                .order_by("start_time")
                .first()
            )
        )()
        ...
        if not latest_booking:
            return {"success": False, "message": "You have no active bookings to modify."}
        ...
        if not (new_date and new_start_time and new_end_time):
            return {"success": False, "message": "Please specify the new date and time for your booking modification."}
        ...
        return {"success": False, "message": f"Could not process modification: {e!s}"}
...
async def handle_cancel_booking(user, session_ctx: dict):
    ...
    is_auth = await sync_to_async(lambda: user.is_authenticated if user else False)()
    if not is_auth:
        return {"success": False, "message": "Please log in to cancel your booking."}
    ...
    try:
        ...
        latest_booking = await sync_to_async(
            lambda: (
                ...
                .filter(user=user, status="confirmed", start_time__gt=timezone.now())
                .order_by("start_time")
                .first()
            )
        )()
        ...
        if not latest_booking:
            return {"success": False, "message": "You have no active bookings to cancel."}
        ...
        return {"success": False, "message": f"Could not process cancellation: {e!s}"}
