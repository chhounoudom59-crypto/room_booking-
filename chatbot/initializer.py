...
def create_chat_agent():
    ...
    try:
        ...
        print(f"\n{'=' * 70}", file=sys.stderr)
        print("CRITICAL: ChatAgent Initialization Failed", file=sys.stderr)
        print(f"{'=' * 70}", file=sys.stderr)
        print(f"Error: {e}", file=sys.stderr)
        print("\nTroubleshooting steps:", file=sys.stderr)
        print("1. Check .env file exists in project root", file=sys.stderr)
        print("2. Verify GROQ_API_KEY is set (get from https://console.groq.com/keys)", file=sys.stderr)
        print("3. Verify API key is valid (starts with 'gsk_')", file=sys.stderr)
        print("4. Check internet connection", file=sys.stderr)
        print("5. Check Groq API status: https://status.groq.com/", file=sys.stderr)
        print(f"{'=' * 70}\n", file=sys.stderr)
        raise
