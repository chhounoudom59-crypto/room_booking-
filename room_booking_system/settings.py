...
try:
    ...
    warnings.warn(
        "pymysql not installed; MySQLdb shim not applied. Install pymysql if using MySQL.", UserWarning, stacklevel=2
    )
...
if DEBUG and "10.0.2.2" not in ALLOWED_HOSTS and "*" not in ALLOWED_HOSTS:
...
TELEGRAM_ADMIN_CHAT_IDS = [
    chat_id.strip() for chat_id in config("TELEGRAM_ADMIN_CHAT_IDS", default="").split(",") if chat_id.strip()
]
